...
let LOGLEVEL = process.env.LOGLEVEL.split('&');
let logLevel = LOGLEVEL.find(level => level == 'log');
let warnLevel = LOGLEVEL.find(level => level == 'warn');
let errLevel = LOGLEVEL.find(level => level == 'error');
