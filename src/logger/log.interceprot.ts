...
export class LoggingInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    const incommingMessage = context.getArgByIndex(0);
    const logIncommingMessage = {
    ...
    const responseStatus = context.getArgByIndex(1).statusCode;
    try {
      logFunc(logIncommingMessage, responseStatus, process.env.REQRESLOGFILE);
    } catch (error) {
      console.log(
        `\x1b[31m%s\x1b[0m`,
        `Message ${incommingMessage} could not be logged!`,
      );
    }
...
async function logFunc(
  logIncommingMessage: object | string,
