...
export class AuthService {
  ...
  async generateToken(
    ...
    return await this.jwtService.signAsync(payload, {
      ...
      expiresIn: `${Number(currentRefreshTokenExpiresAt)}`,
    });
  }
  ...
  async update(updatePasswordDto: UpdatePasswordDto) {
    ...
    let refreshToken;
    if (newPassword == undefined) return STATUS.BADREQUEST;
    const user = await prisma.user.findFirst({
      where: { login: updatePasswordDto.login },
    });
    if (user == null) {
      return STATUS.NOTFOUND;
    }
    const checkPass = await bcrypt.compare(newPassword, user.password);
    const accessTokenLifeTime = new Date(
      Date.now() +
        3600 * 1000 * Number(process.env.REFRESH_TOKEN_LIFETIME) -
        100,
    );
    ...
    if (
      ...
      user.refresTockenLifeTime == undefined
    ) {
      const refreshTokenLifeTime = new Date(
        Date.now() + 3600 * 1000 * Number(process.env.REFRESH_TOKEN_LIFETIME),
      );
