...
export class AuthController {
    constructor (private readonly authService: AuthService) {}
      ...
      async update( @Body() updatePasswordDto: UpdatePasswordDto,) {
        ...
        const serviceAnswer: STATUS | unknown = await this.authService.update(
          updatePasswordDto,
        );
      ...
      async refresh( @Body() refreshToken: any){
        if (refreshToken.refreshToken !== undefined) refreshToken = refreshToken.refreshToken;
        if (refreshToken == undefined || refreshToken == '' || refreshToken.length < 10 || refreshToken.length == undefined) {
          throw new UnauthorizedException(`body does not contain required fields or they are incorrect!`);
        }
        const serviceAnswer: STATUS | unknown = await this.authService.refresh(
          refreshToken,
        );
        if (serviceAnswer == STATUS.NOTFOUND) {
